//! CL-IBE Scheme from Cheng-Comley 
//! * From: "[Efficient Certificateless Public Key Encryption](https://eprint.iacr.org/2005/012.pdf)"
...
// Public key generated by the KGC and used to encrypt messages 
...
// Master secret key generated by  the KGC and used to extract partial user secret key d_a
#[derive(Clone, Debug)]
pub struct MasterSecretKey{
    s:Fr,
}

// Partial user secret key generated by the KGC 
#[derive(Clone, Debug)]
pub struct UserSecretKey{  //Known by the KGC and the User
    d_a: G2Affine,
}

// Partial user secret key generated by the user itself and not known by the KGC
pub struct UserPrivateKey{  //Known only by user
    t_a:Fr,
}

// User public key generated by the user itself used to encrypt messages in addition to its identity
#[derive(Clone, Debug)]
...
impl CLIBE {
    ...
    // Generates public parameters and master secret key
    ...
    // Generates partial user secret key, run by KGC
    ...
    // Generates partial user secret key and user public key, run by the user
    ...
    // Encrypt a message using public parameters, user public key and identity
    /// The implementation is described as:
    /// ```Given message, id, p, p_pub, n_a
    ///    1. Pick a random σ ∈ {0,1}^n
    ///    2. r = h3(sigma,message)
    ///    3. q_a = h1(id)
    ///    4. g_r = ê(p_pub,q_a)^r where ê is a bilinear pairing map ê:G1 x G2 -> Gt 
    ///    5. f = r*n_a
    ///    6. return cipher = (r*p, g^r, message⊕h4(σ))
    /// ```
    fn encrypt<R:Rng+CryptoRng>(
        ...
        // Convert projective coordinates to stock variables into affines one 
    ...
    // Decrypt ciphertext the 2 user secret keys, one generated by KGC and one by the user
    /// The implementation is described as:
    /// ```Given ciphertext (u, v, w), p, p_pub, d_a, t_a
    ///    1. Compute g' = ê(u,d_a), f' = t_a*U, σ' = v⊕h2(u,g',f') 
    ///    2. m' = w⊕h4(σ') and r'=h3(σ',m')
    ///    3. q_a = h1(id)
    ///    4. return m' as plaintext if u=r'*p else error
    /// ```
    fn decrypt(
        cipher : &CipherText,
        usk : &UserSecretKey,
...
mod tests {
    ...
    fn test_pairing_identity() {
        ...
        // Côté decrypt : g' = e(U, d_A) where d_A = s·Q_A
